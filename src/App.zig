...
pub fn deinit(self: *App) void {
    ...
    // Clean up our theme dispenser queue
...
test "ThemeSlotDispenser empty pool returns zero" {
...
test "ThemeSlotDispenser single slot returns zero" {
    const testing = std.testing;
    var d = ThemeSlotDispenser.init();
    defer d.deinit(testing.allocator);
...
test "ThemeSlotDispenser reshuffles on pool size change" {
    ...
    try testing.expectEqual(@as(u16, 5), d.pool_len);
}
